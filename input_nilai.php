...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($data !== null) { // Check if JSON was successfully decoded
        ...
        if ($kelas_query && mysqli_num_rows($kelas_query) > 0) {
            ...
            $check_query = mysqli_query($conn, "SELECT * FROM nilai_siswa WHERE nisn = '$nisn' AND id_mapel = '$idmap' AND kode_kelas = '$kode_kelas'");
            if (mysqli_num_rows($check_query) > 0) {
                ...
                $query = mysqli_query($conn, "INSERT INTO nilai_siswa (id_tahun, nisn, kode_kelas, id_mapel, nilai_uts, nilai_uas, guru_pengajar) 
                VALUES ('$tahun_ajar', '$nisn', '$kode_kelas', '$idmap',  '$nilai_uts', '$nilai_uas', '$guru')");
            
                if ($query) {
                    $response['success'] = true;
                    $response['message'] = 'Nilai Ujian tersimpan';
